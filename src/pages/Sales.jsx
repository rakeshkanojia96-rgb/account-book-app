...
function Sales() {
  ...
  useEffect(() => {
    // OPTION A: Amount Received is GST-inclusive selling amount.
    // Profit = Amount Received - (Purchase Cost incl. purchase GST) - Selling Expenses - GST on selling.

    const quantity = Number(formData.quantity) || 0
    const unitCost = Number(formData.unit_price) || 0
    const purchaseGstPercentage = Number(formData.purchase_gst_percentage) || 0
    const saleGstPercentage = Number(formData.gst_percentage) || 0
    const sellingExpenseAmount = Number(formData.selling_expense_amount) || 0
    const amountReceived = Number(formData.amount_received) || 0

    // Purchase side: cost including purchase GST
    const purchaseBase = quantity * unitCost
    const purchaseGstAmount = purchaseBase * (purchaseGstPercentage / 100)
    const totalPurchaseCost = purchaseBase + purchaseGstAmount

    // Sale side: split Amount Received into base + GST using Sale GST %
    let saleBaseAmount = amountReceived
    let saleGstAmount = 0

    if (amountReceived > 0 && saleGstPercentage > 0) {
      saleBaseAmount = amountReceived / (1 + saleGstPercentage / 100)
      saleGstAmount = amountReceived - saleBaseAmount
    }

    const profitAmount = amountReceived - totalPurchaseCost - sellingExpenseAmount - saleGstAmount

    setFormData(prev => ({
      ...prev,
      amount: Number.isFinite(saleBaseAmount) ? Number(saleBaseAmount.toFixed(2)) : 0,
      gst_amount: Number.isFinite(saleGstAmount) ? Number(saleGstAmount.toFixed(2)) : 0,
      total_amount: Number.isFinite(amountReceived) ? Number(amountReceived.toFixed(2)) : 0,
      profit_amount: Number.isFinite(profitAmount) ? Number(profitAmount.toFixed(2)) : 0
    }))
  }, [formData.quantity, formData.unit_price, formData.purchase_gst_percentage, formData.gst_percentage, formData.amount_received, formData.selling_expense_amount])
  ...
  const handleEdit = (sale) => {
    setFormData({
      ...
      purchase_gst_percentage: sale.purchase_gst_percentage || 0,
  ...
  const handleDownloadTemplate = () => {
    ...
    const sampleRow = [
      ...
      'Sample Product',
      '2',
      '300', // Unit Cost (purchase price per unit)
      '5',   // Purchase GST %
      '18',  // Sale GST % on Amount Received
      '1000', // Amount Received (GST-inclusive)
      '50',   // Selling expense amount
      'Packing, Transport',
      'Online',
    ...
    try {
      ...
      for (let i = 0; i < rows.length; i++) {
        ...
        try {
          ...
          const unitCost = Number(row.unit_price)

          const rawPurchaseGst = row.purchase_gst_percentage === undefined || row.purchase_gst_percentage === null
            ? ''
            : String(row.purchase_gst_percentage).trim().replace('%', '')

          let purchaseGstPercentage = rawPurchaseGst === '' ? 0 : Number(rawPurchaseGst)

          if (!Number.isNaN(purchaseGstPercentage) && purchaseGstPercentage > 0 && purchaseGstPercentage <= 1) {
            purchaseGstPercentage = purchaseGstPercentage * 100
          }

          const rawSaleGst = row.gst_percentage === undefined || row.gst_percentage === null
            ? ''
            : String(row.gst_percentage).trim().replace('%', '')

          let saleGstPercentage = rawSaleGst === '' ? 0 : Number(rawSaleGst)

          if (!Number.isNaN(saleGstPercentage) && saleGstPercentage > 0 && saleGstPercentage <= 1) {
            saleGstPercentage = saleGstPercentage * 100
          }
          ...
          if (
            ...
            Number.isNaN(purchaseGstPercentage) ||
            Number.isNaN(saleGstPercentage) ||
            Number.isNaN(amountReceivedInput) ||
            Number.isNaN(sellingExpenseAmount)
          ) {
            throw new Error('quantity, unit_price, purchase_gst_percentage, gst_percentage, amount_received, and selling_expense_amount must be numbers')
          }
          // Purchase side: cost including purchase GST
          const purchaseBase = quantity * unitCost
          const purchaseGstAmount = purchaseBase * (purchaseGstPercentage / 100)
          ...
          // Sale side: Amount Received is GST-inclusive; split into base + GST using Sale GST %
          const amountReceived = amountReceivedInput
          let baseAmount = amountReceived
          let gstAmount = 0

          if (amountReceived > 0 && saleGstPercentage > 0) {
            baseAmount = amountReceived / (1 + saleGstPercentage / 100)
            gstAmount = amountReceived - baseAmount
          }

          const platformCommission = 0
          const profitAmount = amountReceived - totalPurchaseCost - sellingExpenseAmount - gstAmount
          ...
          const saleRecord = {
            ...
            gst_percentage: saleGstPercentage,
            amount: parseFloat(baseAmount.toFixed(2)),
            gst_amount: parseFloat(gstAmount.toFixed(2)),
            total_amount: parseFloat(amountReceived.toFixed(2)),
  ...
  const totalCost = filteredSales.reduce((sum, sale) => {
    ...
    const unitCost = Number(sale.unit_price) || 0
    const purchaseGstPercentage = Number(sale.purchase_gst_percentage) || 0
    const purchaseBase = qty * unitCost
    const purchaseGstAmount = purchaseBase * (purchaseGstPercentage / 100)
    return sum + purchaseBase + purchaseGstAmount
  }, 0)
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-3">
        ...
        <div className="bg-orange-50 rounded-lg p-3 border border-orange-200">
          <p className="text-xs text-orange-600 mb-1 font-medium">Total Cost (Purchase + GST)</p>
          ...
          <div className="bg-white rounded-xl shadow-xl max-w-2xl w-full max-h-[90vh] overflow-y-auto">
            ...
            <form onSubmit={handleSubmit} className="p-6 space-y-4">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                <div>
                  <label className="block text-sm font-medium text-gray-700 mb-2">
                    Purchase GST % (on Unit Cost)
                  </label>
                  <select
                    value={formData.purchase_gst_percentage}
                    onChange={(e) => setFormData({...formData, purchase_gst_percentage: parseFloat(e.target.value) || 0})}
                    className="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-primary focus:border-transparent outline-none"
                  >
                ...
                <div>
                  ...
                  <select
                    ...
                    value={formData.selling_expense_category
                      ? formData.selling_expense_category.split(',').map(v => v.trim()).filter(Boolean)
                      : []}
                    onChange={(e) => {
                      const selected = Array.from(e.target.selectedOptions).map(o => o.value)
                      setFormData({
                        ...formData,
                        selling_expense_category: selected.join(', ')
                      })
                    }}
                    className="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-primary focus:border-transparent outline-none h-24"
                  >
                    {expenseCategories.map(cat => (
                      <option key={cat.id} value={cat.category_name}>{cat.category_name}</option>
                    ))}
                  </select>
                  <p className="text-xs text-gray-500 mt-1">Hold Ctrl (Windows) or Cmd (Mac) to select multiple expense types</p>
                </div>
              ...
              <div className="bg-gray-50 rounded-lg p-4 space-y-2">
                ...
                <div className="flex justify-between text-sm">
                  <span className="text-gray-600">Sale GST ({formData.gst_percentage}%):</span>
                ...
                {formData.unit_price > 0 && (
                  <div className="flex justify-between text-sm">
                    <span className="text-gray-600">(-) Purchase Cost (incl. GST):</span>
                    <span>
                      ₹{(
                        (Number(formData.unit_price) || 0) * (Number(formData.quantity) || 0) *
                        (1 + (Number(formData.purchase_gst_percentage) || 0) / 100)
